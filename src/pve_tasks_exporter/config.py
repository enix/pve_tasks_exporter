...
def config_from_yaml(yaml):
    ...
    modules = {key: config_module_from_yaml(value) for key, value in yaml.items()}
    invalid = [
        "  - {0}: {1}".format(key, module)
    ...
    if invalid:
        return ConfigInvalid(
            "\n".join(["Invalid module config entries in config file"] + invalid)
        )
...
def config_from_env(env):
    ...
    confvals = {
        confkey: env[envkey] for envkey, confkey in envmap.items() if envkey in env
    }
    ...
    if not confvals:
        return ConfigInvalid("Empty dictionary. No pve API parameters specified.")
